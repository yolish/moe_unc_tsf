...
class UncHead(nn.Module):
    ...
    def forward(self, x):  # x: [bs x nvars x d_model x patch_num]
        x = self.unc_head(x)
        log_sigma = x.permute(0, 2, 1) # non negative and stability
        return log_sigma
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.prob_expert:
            log_sigma = self.unc_head(enc_out)  # z: [bs x nvars x target_window]
            log_sigma = log_sigma[:, :, :N]
            return dec_out, log_sigma
        return dec_out
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            if self.prob_expert:
                dec_out, log_sigma = dec_out
                return dec_out[:, -self.pred_len:, :], log_sigma[:, -self.pred_len:, :]  # [B, L, D]
            else:
