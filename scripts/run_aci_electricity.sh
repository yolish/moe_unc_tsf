#!/bin/bash
# Reproduce Table 1 (ACI-adapted calibration intervals) for electricity (321 channels).
#
#   bash scripts/run_aci_electricity.sh [gpu]
#
...
LOGDIR=logs/aci_electricity; mkdir -p "$LOGDIR"
...
COMMON="--task_name long_term_forecast --model iTransformer --features M \
        ...
        --data custom --root_path ./data/long_term_forecast/electricity/ --data_path electricity.csv \
        --enc_in 321 --dec_in 321 --c_out 321 --learning_rate 0.001 \
...
for seed in "${SEEDS[@]}"; do
    run "electricity_s${seed}_mog" $COMMON --seed $seed --model_id test --num_experts 3 --prob_expert \
        --do_aci_cpvs_calibration --do_aci_aleatoric_scale_g001_calibration --do_aci_cp_calibration

    run "electricity_s${seed}_sg" $COMMON --seed $seed --model_id test --num_experts 1 --prob_expert \
        --do_aci_cpvs_calibration

    run "electricity_s${seed}_cqr" $COMMON --seed $seed --model_id cqr --num_experts 1 --use_quantile_loss \
