...
class UncHead(nn.Module):
    def __init__(self, d_model, pred_len, arc_type="mlp", per_time=True, per_var=False):
        super().__init__()
        self.orig_pred_len = pred_len
        if not per_time:
            pred_len = 1
        if arc_type == "linear":
    ...
    def forward(self, x):  # x: [bs x nvars x d_model x patch_num]
        ...
        if sq_sigma.shape[1] != self.orig_pred_len:
            sq_sigma = sq_sigma.repeat(1, self.orig_pred_len, 1)
         if not self.per_var:
            num_vars = sq_sigma.shape[2]
            sq_sigma = sq_sigma.mean(dim=2)
            sq_sigma = sq_sigma.unsqueeze(2).repeat(1, 1, num_vars)
        return sq_sigma
