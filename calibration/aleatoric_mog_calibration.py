...
class AleatoricMOGCalibrator:
    ...
    def fit(self, preds, ale_var, epi_var, targets):
        ...
        scores = (residuals_sq - epi_var) / (ale_var + 1e-8)
        
        ratio = ale_var / (epi_var + 1e-6)
        #print(f"Aleatoric/Epistemic Ratio: {ratio}")
        print(f"Aleatoric/Epistemic Max Ratio: {np.max(ratio)}")
        print(f"Aleatoric/Epistemic Min Ratio: {np.min(ratio)}")
        print(f"Aleatoric/Epistemic Mean Ratio: {np.mean(ratio)}")
        print(f"Aleatoric/Epistemic Median Ratio: {np.median(ratio)}")
        print(f"Aleatoric/Epistemic Std Ratio: {np.std(ratio)}")
        print(f"Aleatoric/Epistemic 25th Percentile Ratio: {np.percentile(ratio, 25)}")
        print(f"Aleatoric/Epistemic 75th Percentile Ratio: {np.percentile(ratio, 75)}")
        percentage = np.mean(ratio < 50) * 100
        print(f"Percentage of samples where Aleatoric/Epistemic Ratio < 50: {percentage:.2f}%")
    ...
    def predict_one_step(self, pred_t, ale_var_t, epi_var_t):
        ...
        calibrated_variance = q_squared * ale_var_t + epi_var_t
    ...
    def update(self, pred_t, ale_var_t, epi_var_t, target_t):
        ...
        new_score = (res_sq - epi_var_t) / (ale_var_t + 1e-8)
