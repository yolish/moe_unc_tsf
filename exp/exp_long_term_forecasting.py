...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def calibrate_cp(self, setting):
        ...
        calibrator = AdaptiveCP(alpha=0.1, window_size=500)
    ...
    def calibrate_aleatoric_mog(self, setting):
        ...
        calibrator = AleatoricMOGCalibrator(alpha=0.1, window_size=500)
    ...
    def calibrate_aleatoric_only(self, setting):
        ...
        def get_aleatoric_data(flag):
            data_set, loader = self._get_data(flag=flag) 
            preds_list, ale_list, trues_list = [], [], []
            ...
            with torch.no_grad():
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(loader):
                    ...
                    ale_unc, _, _ = self.calc_aleatoric_epistermic_uncertainty(
                        outputs, agg_outputs, expert_unc, expert_weights
                    )
                    pred = agg_outputs
        ...
        val_preds, val_ale, val_trues, _ = get_aleatoric_data('val')
        calibrator.fit(val_preds, val_ale, val_trues)

        test_preds, test_ale, test_trues, test_data_obj = get_aleatoric_data('test')
        ...
        for t in range(n_test):
            ...
            if last_q is None or window_changed:
                lower, upper, curr_q = calibrator.predict_one_step(test_preds[t], test_ale[t])
                last_q = curr_q
            else:
                curr_q = last_q 
                calibrated_variance = curr_q * test_ale[t]
            ...
            t_update = t - pred_len
            if t_update >= 0:
                calibrator.update(test_preds[t_update], test_ale[t_update], test_trues[t_update])
        ...
        with open("result_calibration_aleatoric_only.txt", 'a') as f:
            f.write(f"{setting} (Aleatoric ONLY Calibration)\n")
            f.write(f"q_sq_mean: {np.mean(q_history):.4f}, Coverage: {coverage:.4f}, Width: {width:.4f}\n\n")
