...
domains_to_names = {"national-illness":"ILI",
...
for domain in domains:
    ...
    for i, pl in enumerate(pred_lens):
        ...
        for e in experts:
            ...
            if prob_moe_stan.empty:
               prob_stan_moe_mse = -1 
            else:
                prob_stan_moe_mse = to_value(prob_moe_stan["mse"].values)
            ...
            if prob_stan_moe_mse == -1 or prob_moe_mse == -1:
                if prob_stan_moe_mse == -1:
                    row.append("M")
                else:
                    row.append("{:.3f}".format(prob_stan_moe_mse))
                if prob_moe_mse == -1:
                    row.append("M")
                else:
                    row.append("{:.3f}".format(prob_moe_mse))
            else:
                if prob_stan_moe_mse < prob_moe_mse:
                    row.append("\\textbf{" + "{:.3f}".format(prob_stan_moe_mse) + "}")
                    row.append("{:.3f}".format(prob_moe_mse))
                elif prob_stan_moe_mse == prob_moe_mse:
                    row.append("\\textbf{" + "{:.3f}".format(prob_stan_moe_mse) + "}")
