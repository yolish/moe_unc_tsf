...
from calibration.archive.aleatoric_mog_calibration import AleatoricMOGCalibrator
from calibration.archive.aleatoric_mog_calibration_second_option import AleatoricMOGCalibratorSecondOption
from calibration.aleatoric_only_calibration import AleatoricOnlyCalibrator
from calibration.aleatoric_scale_calibration import AleatoricScaleCalibrator
from calibration.aci_aleatoric_scale_calibration import ACIAleatoricScaleCalibrator
from calibration.adaptive_variance_calibration import AdaptiveVarianceCalibrator
from calibration.adaptive_window_calibration import AdaptiveWindowAleatoricScaleCalibrator
from calibration.cp_dvs_calibration import CPDVSCalibrator, MoGPrediction
from calibration.moecp_calibration import MoECPCalibrator
...
from calibration.aci_aleatoric_only_calibration import ACIAleatoricOnlyCalibrator
from calibration.aci_moecp_calibration import ACIMoECPCalibrator
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def calibrate_aleatoric_mog(self, setting):
        print(">>>>>>> Start Calibration (Aleatoric MoG) >>>>>>>>>>>")
        return self._run_separated_calibration(
            setting,
            AleatoricMOGCalibrator(alpha=0.1, window_size=1000),
            use_epistemic=True,
            title="Aleatoric MoG CP",
            result_file="result_calibration_aleatoric_mog.txt")

    def calibrate_aleatoric_mog_second_option(self, setting):
        print(">>>>>>> Start Calibration (Aleatoric MoG, second option) >>>>>>>>>>>")
        return self._run_separated_calibration(
            setting,
            AleatoricMOGCalibratorSecondOption(alpha=0.1, window_size=1000),
            use_epistemic=True,
            title="Aleatoric MoG CP (second option)",
            result_file="result_calibration_aleatoric_mog_v2.txt")

    def calibrate_aleatoric_only(self, setting):
        print(">>>>>>> Start Calibration (Aleatoric Only) >>>>>>>>>>>")
        return self._run_separated_calibration(
            setting,
            AleatoricOnlyCalibrator(alpha=0.1, window_size=1000),
            use_epistemic=False,
            title="Aleatoric Only CP",
            result_file="result_calibration_aleatoric_only.txt")

    def calibrate_aleatoric_scale(self, setting):
        print(">>>>>>> Start Calibration (Aleatoric Scale) >>>>>>>>>>>")
        return self._run_separated_calibration(
            setting,
            AleatoricScaleCalibrator(alpha=0.1, window_size=1000),
            use_epistemic=True,
            title="Aleatoric Scale CP",
            result_file="result_calibration_aleatoric_scale_tsf.txt")
    ...
    def calibrate_adaptive_window(self, setting):
        print(">>>>>>> Start Calibration (Adaptive Window Aleatoric Scale) >>>>>>>>>>>")
        calibrator = AdaptiveWindowAleatoricScaleCalibrator(
            alpha=0.1, pred_len=self.args.pred_len)
        coverage, width = self._run_separated_calibration(
            setting,
            calibrator,
            use_epistemic=True,
            title="Adaptive Window CP",
            result_file="result_calibration_adaptive_window_tsf.txt",
            extra_fields=lambda: f", window: {calibrator.window_size}")
        print(f"Selected window size = {calibrator.window_size} "
              f"(tuned on {calibrator.n_eval_} held-out val steps)")
        return coverage, width

    def calibrate_adaptive_variance(self, setting):
        print(">>>>>>> Start Calibration (Adaptive Variance) >>>>>>>>>>>")
        calibrator = AdaptiveVarianceCalibrator(alpha=0.1, window_size=1000)
        coverage, width = self._run_separated_calibration(
            setting,
            calibrator,
            use_epistemic=True,
            title="Adaptive Variance CP",
            result_file="result_calibration_adaptive_variance_tsf.txt",
            extra_fields=lambda: f", r: {calibrator.r:.4f}")
        print(f"Selected epistemic ratio r = {calibrator.r:.4f}")
        return coverage, width
    ...
    def calibrate_moecp(self, setting):
        """MoECP: gating-localized weighted conformal prediction.

        Uses the same rolling window and delayed-update protocol as calibrate_cpvs: the
        window is seeded on the validation split and each test origin is fed back
        pred_len steps late, so no target is used before it would have been observable.
        Unlike the CP-VS family the quantile must be recomputed at every origin, because
        it depends on the test point's own gate distribution -- that localization is the
        method. `--moecp_recompute_every` trades that fidelity for speed.
        """
        print(">>>>>>> Start Calibration (MoECP) >>>>>>>>>>>")
        alpha = float(getattr(self.args, 'moecp_alpha', 0.1))
        temperature = float(getattr(self.args, 'moecp_temperature', 100.0))
        recompute_every = int(getattr(self.args, 'moecp_recompute_every', 1))
        ...
        calibrator = MoECPCalibrator(alpha=alpha, temperature=temperature,
                                     window_size=1000, recompute_every=recompute_every,
                                     seed=getattr(self.args, 'seed', 0))

        val_preds, val_gates, val_trues, _ = self._collect_gating('val')
        calibrator.fit(val_preds, val_gates, val_trues)
        test_preds, test_gates, test_trues, test_data_obj = self._collect_gating('test')

        n_test = test_preds.shape[0]
        pred_len = self.args.pred_len
        workers = int(getattr(self.args, 'moecp_workers', 1))

        if workers > 1:
            q_all = _moecp_q_parallel(calibrator, test_preds, test_gates, test_trues,
                                      pred_len, workers)
        else:
            q_all = np.empty_like(test_preds)
            for t in range(n_test):
                _, _, curr_q = calibrator.predict_one_step(test_preds[t], test_gates[t])
                q_all[t] = curr_q
                t_update = t - pred_len
                if t_update >= 0:
                    calibrator.update(test_preds[t_update], test_gates[t_update],
                                      test_trues[t_update])

        final_lowers = test_preds - q_all
        final_uppers = test_preds + q_all
        with np.errstate(invalid='ignore'):
            q_history = [np.mean(q[np.isfinite(q)]) if np.any(np.isfinite(q)) else np.nan
                         for q in q_all]

        # MoECP can return unbounded intervals by design (the target level falling inside
        # the test point's mass at +inf). Those are counted, and excluded from the width
        # average, which would otherwise be inf and uninformative.
        finite = np.isfinite(final_uppers - final_lowers)
        frac_infinite = float(np.mean(~finite))

        if test_data_obj.scale and self.args.inverse:
            print("Applying Inverse Transform to metrics...")
            shape = final_lowers.shape
            with np.errstate(invalid='ignore'):
                final_lowers = test_data_obj.inverse_transform(
                    final_lowers.reshape(shape[0] * shape[1], -1)).reshape(shape)
                final_uppers = test_data_obj.inverse_transform(
                    final_uppers.reshape(shape[0] * shape[1], -1)).reshape(shape)
            test_trues = test_data_obj.inverse_transform(
                test_trues.reshape(shape[0] * shape[1], -1)).reshape(shape)

        coverage = np.mean((test_trues >= final_lowers) & (test_trues <= final_uppers))
        widths = (final_uppers - final_lowers)[finite]
        width = float(np.mean(widths)) if widths.size else float('inf')

        print(f"\nMoECP Results (alpha={alpha}, tau={temperature}, delayed):")
        print(f"Mean q (finite): {np.nanmean(q_history):.4f}")
        print(f"Coverage: {coverage:.4f}")
        print(f"Avg Width (finite only): {width:.4f}")
        print(f"Unbounded intervals: {frac_infinite:.2%}")

        folder_path = './results/' + setting + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)

        with open("result_calibration_moecp_tsf.txt", 'a') as f:
            f.write(f"{setting} (MoECP delayed, alpha={alpha}, tau={temperature})\n")
            f.write(f"q_mean: {np.nanmean(q_history):.4f}, Coverage: {coverage:.4f}, "
                    f"Width: {width:.4f}, Unbounded: {frac_infinite:.4f}\n\n")

        return coverage, width

    def calibrate_cp_dvs(self, setting):
        """CP-DVS: split-conformal with a scale learned from the MoG variance decomposition.

        Unlike the other calibrators here this one is *static* split conformal (fit on val,
        evaluate on test) rather than online with delayed updates, because its scale model
        has to be fit on a block disjoint from the one supplying the conformal quantile.
        `_collect_separated_uncertainty` already returns exactly the law-of-total-variance
        components CP-DVS consumes.
        """
        print(">>>>>>> Start Calibration (CP-DVS) >>>>>>>>>>>")
        alpha = float(getattr(self.args, 'cp_dvs_alpha', 0.1))

        path = os.path.join(self.args.checkpoints, setting, 'checkpoint.pth')
        if os.path.exists(path):
            # map_location so a GPU-saved checkpoint still loads on a CPU-only device.
            self.model.load_state_dict(torch.load(path, map_location=self.device))
        self.model.eval()

        val_preds, val_w, val_b, val_trues, _ = self._collect_separated_uncertainty('val')
        test_preds, test_w, test_b, test_trues, test_data_obj = self._collect_separated_uncertainty('test')

        calibrator = CPDVSCalibrator(alpha=alpha)
        calibrator.calibrate(MoGPrediction.from_decomposition(val_preds, val_w, val_b), val_trues)
        lower, upper = calibrator.predict_intervals(
            MoGPrediction.from_decomposition(test_preds, test_w, test_b))

        if test_data_obj.scale and self.args.inverse:
            print("Applying Inverse Transform to metrics...")
            shape = lower.shape
            lower = test_data_obj.inverse_transform(lower.reshape(shape[0] * shape[1], -1)).reshape(shape)
            upper = test_data_obj.inverse_transform(upper.reshape(shape[0] * shape[1], -1)).reshape(shape)
            test_trues = test_data_obj.inverse_transform(test_trues.reshape(shape[0] * shape[1], -1)).reshape(shape)

        coverage = np.mean((test_trues >= lower) & (test_trues <= upper))
        width = np.mean(np.abs(upper - lower))
        report = calibrator.fit_report_

        print(f"\nCP-DVS Results (alpha={alpha}):")
        print(f"beta: {np.round(calibrator.beta, 4).tolist()}")
        print(f"Pinball on fit split - standard_cp: {report['pinball_standard_cp']:.5f}, "
              f"cpvs: {report['pinball_cpvs']:.5f}, fitted: {report['pinball_fitted']:.5f}")
        print(f"Mean q: {report['q_hat_mean']:.4f}")
        print(f"Coverage: {coverage:.4f}")
        print(f"Avg Width: {width:.4f}")
        ...
        with open("result_calibration_cp_dvs.txt", 'a') as f:
            f.write(f"{setting} (CP-DVS, alpha={alpha})\n")
            f.write(f"q_mean: {report['q_hat_mean']:.4f}, Coverage: {coverage:.4f}, Width: {width:.4f}, "
                    f"beta: {np.round(calibrator.beta, 4).tolist()}\n\n")

        return coverage, width

    # ------------------------------------------------------------------
    # Rolling-window retrained CQR
    # ------------------------------------------------------------------
    ...
    def calibrate_aci_aleatoric_only(self, setting):
        """ACI on the aleatoric-only variance-scaling CP.

        The only member of the family that needs no dedicated loop: the base calibrator
        already exposes a real interval_from_q seam and _run_separated_calibration calls it
        on cache hits, so the served-level record stays aligned without dropping the cache.
        """
        print(">>>>>>> Start Calibration (ACI Aleatoric Only) >>>>>>>>>>>")
        calibrator = ACIAleatoricOnlyCalibrator(
            alpha=self.args.aci_alpha, gamma=self.args.aci_gamma, window_size=1000)
        coverage, width = self._run_separated_calibration(
            setting,
            calibrator,
            use_epistemic=False,
            title="ACI Aleatoric Only CP",
            result_file="result_calibration_aci_aleatoric_only_tsf.txt",
            extra_fields=calibrator.aci.result_suffix)
        print(calibrator.aci.summary_line())
        return coverage, width
    ...
    def calibrate_aci_moecp(self, setting):
        """ACI on MoECP: the gate-localized quantile read off at a per-cell 1 - alpha_t.

        Serial only. --moecp_workers runs whole origin loops in child processes over channel
        slices, which would need alpha_t sliced per worker and the terminal state shipped
        back; ACI also makes origins sequentially dependent through alpha_t. Deferred rather
        than half-done, so the bit-identity check against the base stays meaningful.
        """
        print(">>>>>>> Start Calibration (ACI MoECP) >>>>>>>>>>>")
        alpha = float(self.args.aci_alpha)
        gamma = float(self.args.aci_gamma)
        temperature = float(getattr(self.args, 'moecp_temperature', 100.0))
        recompute_every = int(getattr(self.args, 'moecp_recompute_every', 1))

        workers = int(getattr(self.args, 'moecp_workers', 1))
        if workers > 1:
            print(f"ACI MoECP: --moecp_workers={workers} is not supported, running serially.")
        ...
        calibrator = ACIMoECPCalibrator(alpha=alpha, gamma=gamma, temperature=temperature,
                                        window_size=1000, recompute_every=recompute_every,
                                        seed=getattr(self.args, 'seed', 0))
        ...
        print(f"\nACI MoECP Results (alpha={alpha}, tau={temperature}, gamma={gamma}, delayed):")
        print(f"Mean q (finite): {np.nanmean(q_history):.4f}")
        print(f"Coverage: {coverage:.4f}")
        print(f"Avg Width (finite only): {width:.4f}")
        print(f"Unbounded intervals: {frac_infinite:.2%}")
        ...
        with open("result_calibration_aci_moecp_tsf.txt", 'a') as f:
            f.write(f"{setting} (ACI MoECP delayed, alpha={alpha}, gamma={gamma}, tau={temperature})\n")
            f.write(f"q_mean: {np.nanmean(q_history):.4f}, Coverage: {coverage:.4f}, "
                    f"Width: {width:.4f}, Unbounded: {frac_infinite:.4f}"
                    f"{calibrator.aci.result_suffix()}\n\n")
