...
class Gating(nn.Module): 
    def __init__(self, configs, individual=False, per_time=True, per_var=False):
        """
        ...
        self.orig_seq_len = configs.pred_len
        if not per_time:
            configs.pred_len = 1
    ...
    def gating(self, x_enc):
        ...
        if weights.shape[1] != self.orig_seq_len:
            weights = weights.repeat(1, self.orig_seq_len, 1, 1)
        if not self.per_var:
            num_vars = weights.shape[2]
            weights = weights.mean(dim=2)#torch.nn.functional.adaptive_avg_pool1d(weights, dim=2)
            weights = weights.unsqueeze(2).repeat(1, 1, num_vars,1)
