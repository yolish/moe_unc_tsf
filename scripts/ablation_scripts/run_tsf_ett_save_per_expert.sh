...
data_paths=("ETTh2.csv" "ETTh1.csv") 
datasets=("ETTh2" "ETTh1")
pred_lengths=(96)
num_experts=(3)
configurations=(3)
seeds=(2021)
        ...
        for ne in ${num_experts[@]}
            ...
            for config in "${configurations[@]}"
              do
                    ...
                    if [ $config -eq 1 ]; then
                        echo "python -u run.py --task_name long_term_forecast --root_path $root_path --data_path $data_path --model $model_name --data $dataset --pred_len $pred_len --num_experts $ne --save_expert_outputs"
                        python -u run.py \
